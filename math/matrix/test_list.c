...
/*Kors programmet felfritt genom denna funktion sa fungerar det som specificerat (observera att utskriften maste testas manuellt.
  OBS! Innan alla funktioner �r implementerade sa kommer exekveringen fastna i olika asserts*/
...
/*Du beh�ver sj�lv testa funktionen printList, t�nk p� att testa denna b�de p� en befintlig lista och p� en tom lista
  Testa �ven att det g�r att g�ra clearList p� en tom lista utan att programmet krashar*/
void testFunction(List head)
{
    ...
    printList(head, stdout); //testa utskrift
    printf("\n");
    assert(numberOfNodesInList(head) == 5);
    assert(getFirstElement(head) == 2);
    assert(getLastElement(head) == 6);
    //listan best�r nu av 2-3-4-5-6

    //Testar addLast p� existerande lista
    addLast(&head, 7);
    addLast(&head, 8);
    assert(numberOfNodesInList(head) == 7);
    assert(getFirstElement(head) == 2);
    assert(getLastElement(head) == 8);
    //listan best�r nu av 2-3-4-5-6-7-8

    //Testar removeFirst p� existerande lista
    removeFirst(&head);
    assert(numberOfNodesInList(head) == 6);
    assert(getFirstElement(head) == 3);
    assert(getLastElement(head) == 8);
    //listan best�r nu av 3-4-5-6-7-8

    //Testar removeLast p� existerande lista
    removeLast(&head);
    assert(numberOfNodesInList(head) == 5);
    assert(getFirstElement(head) == 3);
    assert(getLastElement(head) == 7);
    //listan best�r nu av 3-4-5-6-7
    ...
    addLast(&head, 5); //testar att l�gga till sist i tom lista
    ...
    assert(removeElement(&head, 3) == 0); //testar att ta bort fr�n tom lista
    assert(numberOfNodesInList(head) == 0); //testar att r�kna noderna i en tom lista
    printList(head, stdout); //testa utskrift (tom lista)
