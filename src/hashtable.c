...
{
    mdarray a = ht_to_array(ht);
    // trim memory
    int elementCount = mda_count(&a);
...
mdarray ht_to_array(hashtable* ht)
{
    ...
    if (mda_init(&a, size, ht->freeObject) != size)
    {
        return mda_create_empty();
    }

    // move all elements from the hashtable to an array
    for (int i = 0; i < size; i++)
        if (ht->entries[i] != UNUSED)
            mda_insert(&a, ht->entries[i]);
    return a;
}
...
static int linear_probe(hashtable *ht, void* element, int* collisions)
{
    ...
    mdarray a = mda_create_empty();
    int errcode;
    if ((errcode = mda_init(&a, ht_size(ht), ht->freeObject)) != ht_size(ht))
    {
        return errcode;
    }
    ...
    int elements = mda_count(&a);
    for (int i = 0; i < elements; i++)
    {
        void* e = mda_remove_last(&a);
        ht_insert(ht, e);
    }
