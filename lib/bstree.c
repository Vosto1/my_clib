...
static bstree *find_smallest_right(bstree *bst);
static bstree *find_largest_left(bstree *bst);
...
static void* rm_with_two_children(bstree *bst, int (*compare)(const void *, const void *))
{
    ...
    if (random == 1)
    {
        repl = find_smallest_right(bst);
    }
    else
    {
        repl = find_largest_left(bst);
    }
...
const void * bst_rightmost(const bstree *tree)
{
...
const void * bst_leftmost(const bstree *tree)
{
    bstree *tmp = (bstree *)tree;
    while ((*tmp)->left != NULL)
    {
