...
static void insertionSort(ElementType* arrayToSort, unsigned int size, Statistics* statistics)
{
	...
	{
		tmp = arrayToSort[i];
		for (j = i; greaterThan(j, 0, statistics); j--)
		{
			if (lessThan(tmp, arrayToSort[j - 1], statistics))
			{
...
static void merge(int arr[], int left, int right, int mid, Statistics* statistics)
{
	...
	assert(notEqualTo(arrLeft, NULL, statistics));
	
	arrRight = (int)malloc(sizeof(int) * lenRight);
	assert(notEqualTo(arrRight, NULL, statistics));
	
	int i;
	
	for (i = 0; lessThan(i, lenLeft, statistics) && notEqualTo(arrLeft, NULL, statistics); i++)
	{
		arrLeft[i] = arr[i + left];
	}
	for (i = 0; lessThan(i, lenRight, statistics) && notEqualTo(arrRight, NULL, statistics); i++)
	{
	...
	while (lessThan(i1, lenLeft, statistics) && lessThan(i2, lenRight, statistics))
	{
		if (lessThan(arrLeft[i1], arrRight[i2], statistics))
		{
			swapElements(&(arr[i]), &(arrLeft[i1]), statistics);
			i++;
			i1++;
		}
		else
		{
			swapElements(&(arr[i]), &(arrRight[i2]), statistics);
			i++;
			i2++;
		}
	}

	while (lessThan(i1, lenLeft, statistics))
	{
		swapElements(&(arr[i]), &(arrLeft[i1]), statistics);
		i++;
		i1++;
	}
	while (lessThan(i2, lenRight, statistics))
	{
		swapElements(&(arr[i]), &(arrRight[i2]), statistics);
		i++;
		i2++;
